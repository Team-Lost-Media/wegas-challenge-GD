...
## [RhythmNotifier] lets you define custom signals that emit when a given beat in the audio stream is reached, or that emit every [code]N[/code] beats.  The signals are precisely synchronized with the audio, accounting for output latency.  You can also run the [RhythmNotifier] without playing audio, to generate rhythmic signals without music.
## Note: Beats are 0-indexed to make [RhythmNotifier] easier to use, while musicians are accustomed to counting from beat one.
